...
def get_resolution(video):
    resolution = subprocess.check_output(['Dependencies/ffprobe', '-v', 'error', '-select_streams', 'v:0', '-show_entries', 'stream=height,width', '-of', 'csv=s=x:p=0', video])
...
while video_file_name != "quit":
    ...
    if not os.path.exists(video_input_path):
        ...
        time = input("Enter thumbnail time (in seconds) or a time stamp (MM:SS): ")
        print("Please wait...")

        # output thumbnail
        resolution = get_resolution(video_input_path)
        img_output_path = cwd + '/Thumbnails/' + thumbnail_name + '.jpg'
        subprocess.call(['Dependencies/ffmpeg', '-loglevel', 'quiet', '-i', video_input_path, '-vframes', '1', '-an', '-ss', time, img_output_path])
